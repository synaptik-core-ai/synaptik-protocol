...
- **Integrity Proofs** — Evidence of tamper-detection
- **Authenticity Proofs** — Evidence of issuer identity

**Non-Normative Note:** Various cryptographic techniques exist for generating integrity and authenticity proofs. Implementation choices depend on threat model, performance requirements, and verification infrastructure. Consult cryptographic literature and commercial implementations for specific techniques.
...
**Requirements:**
- Hash functions MUST provide at least 128 bits of collision resistance
- Hash functions MUST be from standardized, well-analyzed families
- Cryptographically broken functions (e.g., MD5, SHA-1) MUST NOT be used

**Non-Normative Note:** Implementations should use modern hash functions from established standards (NIST, ISO). Specific algorithm selection depends on performance requirements, hardware support, and interoperability constraints.

### 6.2 Signature Schemes (If Used)

Systems using digital signatures SHOULD use standard, well-analyzed schemes from established cryptographic standards.

**Requirements:**
- Signature schemes MUST be from recognized standards bodies (NIST, ISO, IETF)
- Key sizes MUST meet current security recommendations
- Implementations SHOULD use constant-time operations to prevent side-channel attacks

**Non-Normative Note:** Consult current cryptographic best practices for specific algorithm selection. Systems with long-term security requirements may need to consider post-quantum readiness.
...
3. System computes digest using configured hash function
4. System reads binding metadata: contracts/.signatures
5. System finds entry for financial.toml with expected digest
6. System compares computed vs. expected digest → MATCH
7. System verifies authenticity proof (if present) → VALID
...
A decision record with integrity proof would include:

- Core decision fields (decision ID, verdict, operation, intent, timestamp)
- Integrity proof metadata (algorithm identifier, proof data, key identifier)
- List of covered fields to enable verification

A third-party verifier can:
1. Reconstruct the authenticated message from covered fields using documented canonicalization
2. Obtain the necessary verification data (public keys, trust anchors, etc.)
3. Apply the appropriate verification algorithm
4. Confirm the record hasn't been tampered with

**Note:** The exact proof format and data structures are implementation-defined. See commercial and reference implementations for specific formats.

## Appendix C: Artifact Manifest Example (Non-Normative)

A manifest file binding artifacts to digests would include:

- Manifest metadata (version, creation timestamp)
- List of artifacts with their names, digest algorithm identifiers, and digest values
- Optional signature covering the manifest for authenticity verification

Systems load the manifest, verify its authenticity (if signed), then verify each artifact against its expected digest before use.

**Note:** The exact manifest format (TOML, JSON, YAML, etc.) and structure are implementation-defined. The key requirement is that artifacts can be bound to their expected integrity digests.
